...
<template>
  <div class="space-y-8">
    <div v-if="store.loading.value" class="space-y-2 panel p-4 ">
    ...
    <template v-else>
      <section>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="panel p-4 ">
            <p class="text-sm text-muted">Total de cartes (collection + decks inclus)</p>
            <p class="text-2xl font-semibold">{{ stats.total }}</p>
          </div>
          <div class="panel p-4 ">
            <p class="text-sm text-muted">Cartes uniques</p>
            <p class="text-2xl font-semibold">{{ stats.unique }}</p>
          </div>
        </div>

        <div class="mt-4 space-y-2 panel p-4 ">
          <p class="mb-2 text-sm font-medium text-muted">Répartition par identité de couleur</p>
          <div v-for="color in COLOR_ORDER" :key="color" class="flex items-center gap-3">
            <span class="w-20 shrink-0 text-xs text-muted">{{ COLOR_LABELS[color] }}</span>
            ...
            <span class="w-10 shrink-0 text-right text-xs text-muted">{{ colorBreakdown[color] }}</span>
          </div>
          <div class="flex items-center gap-3">
            <span class="w-20 shrink-0 text-xs text-muted">Multicolore</span>
            <div class="h-2 flex-1 overflow-hidden rounded-full surface-alt">
              <div
            ...
            <span class="w-10 shrink-0 text-right text-xs text-muted">{{ colorBreakdown.multi }}</span>
          </div>
          <div class="flex items-center gap-3">
      ...
      <section>
        ...
        <p v-if="store.decks.value.length === 0" class="text-sm text-muted">
        ...
        <ul v-else class="space-y-2">
          <li
            ...
            :key="deck.id"
            class="flex flex-wrap items-center justify-between gap-3 panel px-4 py-3 "
          >
            <div>
      ...
      <section>
        ...
        <input
          ...
          class="mb-3 field max-w-sm"
        >

        <div class="overflow-x-auto panel">
          <table class="w-full text-sm">
            <thead class="surface-alt text-left text-xs uppercase tracking-wide text-muted  dark:text-muted">
              <tr>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-3 py-2 uppercase text-muted">{{ row.setCode }}</td>
                <td class="px-3 py-2">
          ...
          <p v-if="filteredRows.length === 0" class="p-4 text-center text-sm text-muted">
